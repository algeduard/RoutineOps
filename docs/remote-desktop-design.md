...
Удалённый доступ к живому экрану сотрудника — чувствительная операция.
- **attended (РЕАЛИЗОВАНО, по умолчанию):** при `START`, ДО отправки любых кадров,
  хелпер показывает в юзер-сессии модальный запрос (walk, как lockui) «IT запрашивает
  удалённый доступ. Разрешить?» с таймаутом (`consent_windows.go`, дефолт 30с, env
  `ROUTINEOPS_RD_CONSENT_TIMEOUT`). Только явное «Разрешить» → сеанс идёт. Отказ,
  закрытие окна, таймаут или ошибка показа → `RDStatus{USER_DENIED}` (fail-safe) →
  сервер закрывает WS, админ видит «отклонено пользователем». Кадры до согласия НЕ
  передаются.
- **unattended (для серверов/киосков) — TODO:** админ-настройка на устройство/группу
  без запроса; всё равно неблокирующая плашка «идёт сеанс» + аудит. Пока НЕ
  реализовано — доступ всегда attended (безопасный дефолт).
- **Плашка «идёт сеанс» на устройстве в течение сессии — TODO** (persist-баннер поверх
  окон: пользователь должен видеть активность весь сеанс, а не только на старте).
