...
- **Только имя процесса.** Собирается имя foreground-приложения (напр.
  `chrome.exe`), НЕ заголовки окон / URL / содержимое — грубая гранулярность
  снижает инвазивность.
- **Двойной гейт на сервере.** Даже если агент отстал от смены флага, gateway
  повторно проверяет `app_usage_enabled` и отбрасывает данные при выключенном
  сборе (accept-and-drop, без ошибки агенту).
