...
- **Тиеринг чувствительности + отдельные opt-in гейты.** Базовая гранулярность —
  ...
  - `capture_urls` (миграция 040) — полный URL активной вкладки браузера
    ...
    известных браузеров (`isBrowserProcess`) и НИКОГДА из приватных/инкогнито-окон
    (та же эксклюзия `isPrivateBrowsing`, что и для заголовков). Чтение best-effort:
    его ошибка/пустота не ломает сбор имени процесса/заголовка/активности.
- **Инкогнито/приватные окна исключаются.** Заголовок такого окна отбрасывается
  (`sanitizeTitle`→""), URL из него не читается — приватный просмотр трактуется как
  явный сигнал «истории не надо».
